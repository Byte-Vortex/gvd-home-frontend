...
function Slide({ item, index }) {
    ...
    const phoneImage = getImageProps({ alt: "", src: item.phoneBackground, sizes: "100vw", fill: true }).props;
    const desktopImage = getImageProps({ alt: "", src: item.desktopBackground, sizes: "100vw", fill: true }).props;

    return (
        <Link href={item.link} className={clsx('h-full w-full block relative duration-300', !imageLoaded && "opacity-0")}>

            <picture>
                <source media="(max-width: 767px)" srcSet={phoneImage.srcSet} sizes={phoneImage.sizes} />
                <source srcSet={desktopImage.srcSet} sizes={desktopImage.sizes} />
