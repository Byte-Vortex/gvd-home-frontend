...
export default async function Page() {
    ...
    const slug = "food-distribution";
    const [res, cards] = await Promise.all([makeRequestServer("/activities/"), makeRequestServer("/activities/" + slug)])
...
function Card({ data }) {
    return (
        <Link href={data.slug + "/"} className="bg-surface text-on-surface rounded-xl p-4 gap-1 max-w-sm mx-auto w-full space-y-2">
            <h5>{data.title}</h5>
            <ProseInnerHtmlContainer html={data.text} className="line-clamp-2" />
