...
export default async function Page({ params }) {
    ...
    let scripts = null;
    try {
        [data, scripts] = await Promise.all([makeRequestServer("/blogs/author/" + id), getScriptTags("/blogs/authors/" + id + "/")]);
    } catch (err) {
        return notFound();
    }
    return (
        <div className="max-w-[100vw] lg:px-32 mx-auto px-4 py-12 grid lg:grid-cols-10 gap-12">
            <DynamicScripts pathname={"/blogs/authors/" + id + "/"} scripts={scripts} />
            <div className="lg:col-span-3">
                ...
                <h3 className="text-primary my-3">{data.full_name}</h3>
            ...
            <div className="lg:col-span-7 space-y-6">
                ...
                <div className="w-full aspect-h-9 aspect-w-16 rounded-xl relative">
                    <Image fill alt={data.featured_blog.image_alt} className="w-full h-full object-cover rounded-xl" src={data.featured_blog.imageLink} />
                </div>
